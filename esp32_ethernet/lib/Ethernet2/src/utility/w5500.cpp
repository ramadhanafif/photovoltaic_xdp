...
{
    // SPI_CS = ss_pin;
    SPIe.begin(HSPI_SCLK, HSPI_MISO, HSPI_MOSI, HSPI_SS); // moved up to avoid conflict with arduino-esp32 SPI

    SPI_CS = SPIe.pinSS();

    // this will print out 15 if properly configured.
    // Serial.begin(115200);
    // Serial.printf("SPICS PIN IS %d\n",SPI_CS);
...
uint8_t W5500Class::write(uint16_t _addr, uint8_t _cb, uint8_t _data)
{
    ...
    setSS();
    SPIe.transfer(_addr >> 8);
    SPIe.transfer(_addr & 0xFF);
    SPIe.transfer(_cb);
    SPIe.transfer(_data);
    resetSS();
    SPIe.endTransaction();
